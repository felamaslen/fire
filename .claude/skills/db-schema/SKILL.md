...
The database schema lives in `packages/backend/src/db/schema/**` (Drizzle ORM). Migrations are generated by `yarn db:migrate:create` into `packages/backend/src/db/migrations/` and are committed to the repo.
