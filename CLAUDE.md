...
- **File names describe what's inside, not "miscellaneous".** Avoid generically-named files like `utils.ts`, `helpers.ts`, `misc.ts`, `common.ts`, `lib.ts`, `shared.ts` — they turn into dumping grounds. Name the file after the thing it exports: a module exporting `cn` goes in `lib/cn.ts`, not `lib/utils.ts`. If a single file would hold two unrelated helpers, give them two files instead.
- **Prefer `assert(condition, "error message")` over `if (!condition) throw new Error(...)`.** Import `assert` from `node:assert/strict` (`import { strict as assert } from "node:assert"`) and use it for validation, lookup-not-found, precondition, and invariant checks. The `assert(value, msg)` form throws an `AssertionError` with `msg` when `value` is falsy and also narrows the type, so it doubles as a type guard. Reserve `if (...) throw new Error(...)` for cases where the throw isn't a guard on a single boolean (e.g. building a structured error from multiple inputs).
- **Never use `GraphQLError`.** In resolvers throw plain `new Error("…")` (or, for guards, `assert(cond, "…")`); the server's `formatError` already surfaces the `Error.message` to clients verbatim. When a thrown error needs to carry GraphQL `extensions` (e.g. an error code the client switches on), attach them inline: `throw Object.assign(new Error("…"), { extensions: { code: "…" } })` — do not import `GraphQLError`.
...
- **One `useSuspenseQuery` per page on initial load — show a single loading spinner, not one per widget.** The route's top-level component owns a combined query that selects every fragment the children need on first render; each child is a presentational component (fragment-masked) that gets its data via props, not its own suspense query. For refresh (polling, period changes, re-sorts, "load more", etc.) children use their own non-suspense `useQuery` / `refetch` / `fetchMore`. While those in-flight, the child renders its previous data dimmed (`opacity-50 pointer-events-none`) — never re-shows the page spinner or blanks itself. The combined document's variables should match the children's default variables so the cache is warm for free when a child's own `useQuery` first runs.
