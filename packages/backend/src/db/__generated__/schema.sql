...
CREATE TABLE "NetWorthEntryBuckets" (
  "entryId" uuid NOT NULL,
  "bucket" "netWorthHistoryBucket" NOT NULL,
  "amountHomeMinor" bigint NOT NULL,
  "createdAt" timestamp with time zone DEFAULT now() NOT NULL,
  "updatedAt" timestamp with time zone DEFAULT now() NOT NULL,
  CONSTRAINT "NetWorthEntryBuckets_pk" PRIMARY KEY ("entryId", "bucket")
);
...
ALTER TABLE "NetWorthEntryBuckets"
ADD CONSTRAINT "NetWorthEntryBuckets_entryId_NetWorthEntries_id_fk" FOREIGN KEY ("entryId") REFERENCES "public"."NetWorthEntries" ("id") ON DELETE CASCADE ON UPDATE NO ACTION;
...
CREATE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" () RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
  affected uuid[] := ARRAY[]::uuid[];
BEGIN
  IF pg_trigger_depth() > 1 THEN
...
AFTER
UPDATE ON "NetWorthCategoryAssets" REFERENCING OLD TABLE AS old_rows NEW TABLE AS new_rows FOR EACH STATEMENT
...
AFTER
UPDATE ON "NetWorthCategoryLiabilities" REFERENCING OLD TABLE AS old_rows NEW TABLE AS new_rows FOR EACH STATEMENT
EXECUTE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" ();

CREATE TRIGGER "NetWorthCurrencyRates_refreshBuckets_del_trg"
AFTER DELETE ON "NetWorthCurrencyRates" REFERENCING OLD TABLE AS old_rows FOR EACH STATEMENT
EXECUTE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" ();

CREATE TRIGGER "NetWorthCurrencyRates_refreshBuckets_ins_trg"
AFTER INSERT ON "NetWorthCurrencyRates" REFERENCING NEW TABLE AS new_rows FOR EACH STATEMENT
EXECUTE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" ();

CREATE TRIGGER "NetWorthCurrencyRates_refreshBuckets_upd_trg"
AFTER
UPDATE ON "NetWorthCurrencyRates" REFERENCING OLD TABLE AS old_rows NEW TABLE AS new_rows FOR EACH STATEMENT
EXECUTE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" ();

CREATE TRIGGER "NetWorthValueAmounts_refreshBuckets_del_trg"
AFTER DELETE ON "NetWorthValueAmounts" REFERENCING OLD TABLE AS old_rows FOR EACH STATEMENT
EXECUTE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" ();

CREATE TRIGGER "NetWorthValueAmounts_refreshBuckets_ins_trg"
AFTER INSERT ON "NetWorthValueAmounts" REFERENCING NEW TABLE AS new_rows FOR EACH STATEMENT
EXECUTE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" ();

CREATE TRIGGER "NetWorthValueAmounts_refreshBuckets_upd_trg"
AFTER
UPDATE ON "NetWorthValueAmounts" REFERENCING OLD TABLE AS old_rows NEW TABLE AS new_rows FOR EACH STATEMENT
EXECUTE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" ();

CREATE TRIGGER "NetWorthValues_refreshBuckets_del_trg"
AFTER DELETE ON "NetWorthValues" REFERENCING OLD TABLE AS old_rows FOR EACH STATEMENT
EXECUTE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" ();

CREATE TRIGGER "NetWorthValues_refreshBuckets_ins_trg"
AFTER INSERT ON "NetWorthValues" REFERENCING NEW TABLE AS new_rows FOR EACH STATEMENT
EXECUTE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" ();

CREATE TRIGGER "NetWorthValues_refreshBuckets_upd_trg"
AFTER
UPDATE ON "NetWorthValues" REFERENCING OLD TABLE AS old_rows NEW TABLE AS new_rows FOR EACH STATEMENT
EXECUTE FUNCTION "NetWorthEntryBuckets_refreshFromTrigger_fn" ();
